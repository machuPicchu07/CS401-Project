...
public class PGMS {
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {	//Run server perpetually
				...
				//Create a socket object 'client' equal to a connecting socket, wait (block) until a socket has connected
				Socket client = server.accept();

				//ClientHandler object created using the connected socket
				ClientHandler clientSock = new ClientHandler(client);
				
				//A thread is started using the ClientHandler object, handling the client separately
	...
	private static class ClientHandler implements Runnable {
				...
				while ((inMsg = (Message) in.readObject()) != null) {
					...
					if (!loggedIn) {
						if (msgType == MsgTypes.NEWGARAGE) {			//If not logged in and MsgType == NEWGARAGE
							...
							inMsg = new Message(MsgTypes.SUCCESS, garageID);//Create new Message object with MsgType SUCCESS, and current Garage ID
						...
						//Creates new ticket with RECEIVED MsgType w/ Garage ID to send to Client as response
						case NEWTICKET: {				
						...
						//If ticket is found from the two dimensional array, send ticket 'reply' to client
						case LOOKUPTICKET: {
		...
		private void createNewGarage(int garageID) {
			...
			//Both UNPAIDTICKETS & PAIDTICKETS are instantiated and pushed into their respective Two-Dimensional array
		...
		private Ticket lookUpUnpaidTicket(int garageID, Message inMsg) {	
			List<Ticket> tickets = UNPAIDTICKETS.get(garageID);			//Create ticket list using UNPAIDTICKETS 2d array key (garageID)
